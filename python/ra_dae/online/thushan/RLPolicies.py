...
class ContinuousState(Controller):
    ...
    def move(self, i, data, funcs):
        ...
        q_calc_thresh = 15
        balance_thresh = 10
        err_t = np.mean(data['valid_error_log'][-6:-1])
        err_t_minus_1 = np.mean(data['valid_error_log'][-11:-6])
        ...
        state = (data['r_5'][-1], data['neuron_balance'], ma_state('mea_5'), ma_state('mea_10'), ma_state('mea_15'))
        print('current state %f, %f, %f, %f, %f' % (data['r_5'][-1], data['neuron_balance'], ma_state('mea_5'), ma_state('mea_10'), ma_state('mea_15')))
        print('initial_size ',data['initial_size'])

        err_diff = err_t - err_t_minus_1
        curr_err = err_t
        print('Err log: ',err_t_minus_1,' -> ',err_t)
        ...
        # newer to_move eqn
        if random.random() > 0.1:
            to_move = 0.5*np.exp(-(data['neuron_balance']-1.)**2/balance_thresh) * np.abs(err_diff)
        else:
            to_move = 0.5*np.exp(-(data['neuron_balance']-1.)**2/balance_thresh) * np.abs(err_diff) * (1.+curr_err)
