...
class ContinuousState(Controller):
    ...
    def move(self, i, data, funcs):
        ...
        print('Err log: ',data['valid_error_log'][-2],' -> ',data['valid_error_log'][-1])
        ...
        # newer to_move eqn
        if random.random() > 0.1:
            to_move = 0.25*np.exp(-(data['neuron_balance']-1.)**2/5.) * np.abs(err_diff)
        else:
            to_move = 0.25*np.exp(-(data['neuron_balance']-1.)**2/5.) * np.abs(err_diff) * (1.+curr_err)
